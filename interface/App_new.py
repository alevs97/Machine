...
class AppNew(tk.Frame):
    item_selected = False # Ready
    ...
    def frame_east(self):
        ...
        self.label_start_here_button = ttk.Label(frame_east, text="")
    ...
    def frame_nort(self):
        ...
        frame_label_back = tk.Frame(frame_nort, highlightbackground="black", highlightthickness=1, bd=0)
        frame_label_back.grid(row=2, column=0, sticky="nsew")

        self.label_back_button = ttk.Label(frame_label_back, text="")
        ...
        tk.Label(frame_nort, text=str(self.list_global.job_description), borderwidth=2, relief="groove", padx=15, pady=10)\
            .grid(row=2, column=1, sticky="n")

        frame_label_go_to = tk.Frame(frame_nort, highlightbackground="black", highlightthickness=1, bd=0)
        frame_label_go_to.grid(row=2, column=2, sticky="nsew")

        self.label_front_button = ttk.Label(frame_label_go_to, text="")
    ...
    def service_skip(self):
        ...
        try:
            ...
            self.services.skip_here(self.id_panel, self.id_assemble, self.id_part, self.control_back)
    ...
    def service_start_here(self):
        ...
        try:
            self.services.start_here(self.id_panel, self.id_assemble, self.id_part, self.control_back)
        except TypeError:
    ...
    def render_scroll_frame_parts(self):
        ...
        assemblie = panel.list_assemblies[self.id_assemble]

        # Label of ahere are you
        # TODO: ADD Label where are you(Part/Asseblie/Panel )
        tk.Label(self.frame_list_box,
              text="List of PARTS to process in :" + str(panel.get_id()) + "-" + str(panel.get_name()) + "/"
                   + str(assemblie.get_id()) + "-" + str(assemblie.get_name()), height=1).grid(pady=5, row=0, column=1)

        # Creating the listbox
        self.my_list_parts = tk.Listbox(self.frame_list_box)
    ...
    def render_scroll_frame_assemblies(self):
        ...
        tk.Label(self.frame_list_box,
              text="List of ASSEMBLIES to process in " + str(panel.get_id()) + "-" + str(panel.get_name()), height=1). \
            grid(pady=5, row=0, column=1)

        # Creating the listbox
        self.my_list_assemblies = tk.Listbox(self.frame_list_box)
    ...
    def render_scroll_frame_panels(self):
        ...
        tk.Label(self.frame_list_box, text="List of PANELS to process ", height=1).grid(pady=5, row=0, column=1)

        # Creating the listBox
        self.my_list_panels = tk.Listbox(self.frame_list_box)
